    ...
    def design_architecture_task(agent, requirement):
        return Task(
            description=f'Analyze the following infrastructure requirement and design a MODULAR architecture.\n'
                        ...
                        '2. You MUST design the project with a modular layout (Root calling submodules under `modules/` like `modules/networking/`, `modules/storage/`, etc.).\n'
                        '3. You MUST provide a **Mermaid.js diagram string** representing the CLOUD INFRASTRUCTURE TOPOLOGY & DATA FLOW:\n'
                        '   - Visualize actual cloud resources, components, and traffic/data flows (e.g. `Client / App --> S3 Bucket --> Glacier Archive (30 Days)` or `Internet --> ALB --> EKS Nodes --> RDS`).\n'
                        '   - Include cloud service names and relationship labels.\n'
                        '   - NEVER diagram Terraform file names (`main.tf`, `variables.tf`, `outputs.tf`) or directory trees in the Mermaid diagram.\n'
