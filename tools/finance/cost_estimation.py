...
class CostEstimator:
    ...
    def _build_markdown_report(self, result, budget_limit=100.0):
        if "error" in result:
            ...
            report += f"## ⚠️ **Cost Estimation Error**\n"
            report += f"The cost estimation tool encountered an error during execution:\n"
            report += f"- **Error Type:** `{result.get('error')}`\n"
            report += f"- **Details:** `{result.get('details', 'Unknown error')}`\n\n"
            report += f"Please verify your Infracost API key setup, ensure your environment is configured correctly, and check server logs.\n"
